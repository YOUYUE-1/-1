...
//步骤,关键函数,标志位(Flag),姐姐的叮嘱
//1. 挂载,f_mount,1 (立即),不挂载一切免谈，必须检查返回值 FR_OK。
//2. 写文件,f_open,`FA_CREATE_ALWAYS,FA_WRITE`
//3. 读文件,f_open,FA_READ,读取前最好把缓冲区清零 (memset)。
//4. 遍历,f_readdir,N/A,这是一个循环过程，遇到空文件名表示结束。
//5. 卸载,f_mount,NULL,测试结束，释放资源，好习惯。


//如果你想把文件写进那个文件夹里，姐姐教你两个办法，宝宝根据情况选一个哦：
//方案 A：使用绝对路径（推荐，最稳妥 ??）
//不需要切换目录，直接告诉文件系统具体的地址。这就像寄快递写清楚"XX小区XX号楼"一样准确。

//// 修改文件名定义，带上目录路径
//// 注意：FatFs 中路径分隔符通常是 '/'
//const char *TestFileName = "测试目录/SD_TEST.TXT"; 
//// 后面的 f_open 代码不用动
void test_sd_fatfs(void)
{
    ...
    UINT bw, br;                              // 读写字节数计数
    const char *TestFileName = "测试目录/SD_TEST.TXT1"; // 测试文件名
